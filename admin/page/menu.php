	...
	<div class="col-md-6 well" style="height: 415px;">
		...
		<form role="form" method="post" action="?&action=newLienMenu">
			...
			<div class="radio">
				...
				Lien 
				</label><input type="text" class="form-control" name="menuLien" placeholder="ex: http://minecraft.net/">
			</div>
			<div class="radio">
				<label>
	...
	for($i = 0; $i < count($lectureMenu['MenuTexte']); $i++)
	{
		...
		<form class="tab-pane <?php if($i == 0) echo 'active'; ?>" id="editLiens<?php echo $i; ?>" method="post" action="?&action=modifierLien&id=<?php echo $i; ?>">
			...
			<div class="radio">
				...
				<input type="radio" name="methode" value="1" checked>
				Lien 
				</label><input type="text" class="form-control" name="menuLien" placeholder="ex: http://minecraft.net/" value="<?php echo $lectureMenu['MenuLien'][$i]; ?>" name="lienLien">
			</div>
			<div class="radio">
				<label>
				<input type="radio" name="methode" value="2">
	...
	for($i = 0; $i < count($lectureMenu['MenuTexte']); $i++)
	{
			...
			<form role="form" method="post" class="form-changer-liste-deroulante row" action="?&action=editMenuListe">
				...
				<div class="form-group col-md-6 well">
					...
					<div class="radio">
						...
						Lien 
						</label><input type="text" class="form-control" name="menuLien<?php echo $j; ?>" value="<?php echo $lectureMenu['MenuListeDeroulanteLien'][$lectureMenu['MenuTexte'][$i]][$j]; ?>"placeholder="ex: http://minecraft.net/">
					</div>
					<div class="radio">
						<label>
					...
					<a class="btn btn-danger pull-right" href="?&action=supprLienMenuDeroulant&id=<?php echo $i; ?>&id2=<?php echo $j; ?>">Supprimer le lien: <?php echo $lectureMenu['MenuListeDeroulante'][$lectureMenu['MenuTexte'][$i]][$j]; ?></a>
				</div>
				<?php }
				...
				<input type="submit" class="btn btn-warning pull-right" value="Valider mes changements !" />
			</form>
		<?php } 
		if(isset($lectureMenu['MenuListeDeroulante'][$lectureMenu['MenuTexte'][$i]]))
		{
		?>
			...
			<form role="form" method="post" class="form-changer-liste-deroulante" action="?&action=nouveauMenuListeLien&liste=">
				...
				<input type="submit" class="btn btn-warning" value="Valider mes changements" />			
			</form>
        </div>		
        <?php
		$j++;
